...
if [ "$0" = "$BASH_SOURCE" ]; then
    ...
    verify-contract "lib/forward-proxy/src/ForwardProxy.sol:ForwardProxy" "$RWA008_A_OPERATOR" || exit_code=1
    verify-contract "lib/forward-proxy/src/ForwardProxy.sol:ForwardProxy" "$RWA008_A_MATE" || exit_code=1
    verify-contract "lib/mip21-toolkit/src/tokens/RwaToken.sol:RwaToken" "$RWA008" "\"$SYMBOL\"" "\"$NAME\"" || exit_code=1
    verify-contract "lib/mip21-toolkit/src/conduits/RwaOutputConduit2.sol:RwaOutputConduit" "$RWA008_A_OUTPUT_CONDUIT" "$MCD_DAI" || exit_code=1
    verify-contract "lib/mip21-toolkit/src/urns/RwaUrn.sol:RwaUrn" "$RWA008_A_URN" "$MCD_VAT" "$MCD_JUG" "$MCD_JOIN_RWA008_A" "$MCD_JOIN_DAI" "$RWA008_A_OUTPUT_CONDUIT" || exit_code=1
